...
class QPTorqueOptimizer():
  ...
  # @numba.jit(nopython=False, parallel=False, cache=True)
  def compute_mass_matrix(self, foot_positions):
    ...
    for leg_id in range(4):
      ...
      foot_position_skew = np.array([[0, -x[2], x[1]], [x[2], 0, -x[0]],
                                     [-x[1], x[0], 0]])
      mass_mat[3:6,
               leg_id * 3:leg_id * 3 + 3] = self.inv_inertia.dot(foot_position_skew)
    return mass_mat
  ...
  def compute_contact_force(self,
    ...
    # print("Mass Mat time: {}".format(time.time() - time_before))
    G, a = self.compute_objective_matrix(mass_matrix, desired_acc, acc_weight,
                                    reg_weight)
    # print("Obj Mat time: {}".format(time.time() - time_before))
    C, b = self.compute_constraint_matrix(contacts)
    # print("Constraint Mat time: {}".format(time.time() - time_before))
    G += 1e-4 * np.eye(12)
